...
The implementer starts broker mode with one call:
...
The broker returns `status=task`, `task_review=null`, and
...
The broker returns `task_review.status=PASS` and the next task. The implementer
writes and commits:
...
PARENT_TASK_ID: --
ROOT_USER_INPUT_ID: T-DEMO-01-000
DETAIL: Broker verified USER_INPUT_CAPTURE as process-complete.
```
...
The broker issues `SPEC_SPEC`. The implementer creates
...
If the broker returns `task_review.status=PASS`, the implementer records
`BROKER_TASK_REVIEW: PASS`, commits it, and executes `next_task`.
...
```json
{
  ...
  "task_review": {
    ...
    "required_fixes": [],
    "parent_for_broker_review": "J-20260614-100000-008",
    "detail_suggestion": "Broker verified RED task B-000010 as process-complete.",
...
The implementer must commit `BROKER_TASK_REVIEW: PASS` before executing GREEN.

## Broker FAIL example

If process evidence is missing, the broker returns `status=fail` and no
`next_task`.
...
```json
{
  ...
  "task_review": {
    ...
    "parent_for_broker_review": null,
    "detail_suggestion": null,
    "rationale": "The submitted task is not process-complete."
  },
  "next_task": null,
  "rationale": "Broker cannot issue the next task until the submitted task passes process verification."
}
```

The implementer records `BROKER_TASK_REVIEW: FAIL`, commits it, fixes the

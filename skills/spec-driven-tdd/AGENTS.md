...
`.sddtdd_skill/IMPLEMENTATION-PLAN.md`. Do not launch RED, GREEN, test, code, or
implementation work before `IMPLEMENTATION_PLAN_REVIEW: PASS` and the following
process gate. Every RED/GREEN delegation must match exactly one reviewed plan row
and one `TASKS.md` task id.

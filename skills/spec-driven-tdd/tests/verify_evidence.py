#!/usr/bin/env python3
"""Verify committed Spec-Driven TDD journal, plan, and native OMP evidence."""
...
def collect_omp_task_ids(events: Iterable[dict[str, object]]) -> tuple[set[str], set[str]]:
    agents: set[str] = set()
    jobs: set[str] = set()
    for event in events:
        tool_name = event.get("toolName") or event.get("tool_name") or event.get("name")
        if tool_name not in {"task", "hub"}:
            continue
        for obj in nested_dicts(event):
            ...
            for key in ("agentId", "agent_id"):
                if is_real(obj.get(key)):
                    agents.add(str(obj[key]))
            for key in ("jobId", "job_id"):
                if is_real(obj.get(key)):
                    jobs.add(str(obj[key]))
    if not agents or not jobs:
        raise EvidenceError("raw OMP events contain no task agent/job identifiers")
    return agents, jobs
...
def verify_runtime(
    rows: Iterable[dict[str, object]], omp_events: Iterable[dict[str, object]],
    plan_rows: list[PlanRow],
) -> None:
    records = list(rows)
    raw_agents, raw_jobs = collect_omp_task_ids(omp_events)
    for row in records:
        if is_real(row.get("agent_id")) and str(row["agent_id"]) not in raw_agents:
            raise EvidenceError(f"runtime agent_id not found in raw OMP events: {row['agent_id']}")
        if is_real(row.get("job_id")) and str(row["job_id"]) not in raw_jobs:
            raise EvidenceError(f"runtime job_id not found in raw OMP events: {row['job_id']}")
...
def verify(journal_path: Path, orchestrator_log_path: Path, omp_events_path: Path) -> None:
    if not journal_path.is_file() or journal_path.stat().st_size == 0:
        raise EvidenceError(f"missing or empty journal: {journal_path}")
    plan_rows = verify_implementation_plan(journal_path.with_name("IMPLEMENTATION-PLAN.md"))
    ...
    verify_runtime(
        ...
        load_jsonl(omp_events_path, "raw OMP event stream"),
        plan_rows,
    )
...
def main() -> int:
    ...
    parser.add_argument("omp_events", type=Path)
    args = parser.parse_args()
    try:
        verify(args.journal, args.orchestrator_log, args.omp_events)
